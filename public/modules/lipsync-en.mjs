...
class LipsyncEn {
  ...
  convert_tens(num) {
    if (num < 10) {
      return (
        (Number(num) != 0 && num.toString().startsWith('0') ? 'oh ' : '') + this.ones[Number(num)]
      );
  ...
  convertNumberToWords(num, isNotSpecial = false) {
    ...
    if (num == '0') {
      ...
      return (
        this.convertNumberToWords(parts[0], isNotSpecial).trim() +
        ' point ' +
        this.convert_digit_by_digit(parts[1]).trim()
      );
    } else if (num.toString().startsWith('0')) {
      return this.convert_digit_by_digit(num).trim();
    } else if (
      !isNotSpecial &&
      ((num < 1000 && num > 99 && num % 100 !== 0) || (num > 10000 && num < 1000000))
    ) {
      //read area and zip codes digit by digit
      return this.convert_digit_by_digit(num).trim();
    } else if (!isNotSpecial && ((num > 1000 && num < 2000) || (num > 2009 && num < 3000))) {
      //read years as two sets of two digits
      return num % 100 != 0
        ? this.convert_sets_of_two(num).trim()
        : this.convert_tens(num.toString().substring(0, 2)).trim() + ' hundred';
    } else {
  ...
  preProcessText(s) {
    ...
    if (/\d/.test(r)) {
      ...
      r = r
        .replace(/\b(\w*?)(\d+)([A-Za-z]+)\b/g, (match, prefix, numbers, letters) => {
